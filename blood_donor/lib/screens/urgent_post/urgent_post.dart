...
class UrgentPost extends StatefulWidget {
  const UrgentPost({ Key? key }) : super(key: key);

  @override
  State<UrgentPost> createState() => _UrgentPostState();
}

class _UrgentPostState extends State<UrgentPost> {
  ...
  void submitPressed() async {
    ...
    else
    {
      ...
      dynamic result = await AppController().postRequest(urgentText.text, latitude, longitude);
      EasyLoading.dismiss();     
      if (result['Status'] == "Success")
      {
        Get.offAll(HomeScreen(defaultPage: 0));
        Constants.showDialog('Post submitted successfully');//to admin for approval');
      }
      else 
      {
